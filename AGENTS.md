...
- **Frontend**: React 19 + TypeScript + Vite, static-hosted on S3/CloudFront. shadcn/ui (Base UI) components, TanStack Query for server state, TanStack Table v8 for the Patients/Appointments/Audit Log tables, react-router-dom for routing, direct Cognito SRP auth via `amazon-cognito-identity-js`. Typed API client (`openapi-fetch`) generated from the backend's OpenAPI schema — see ADR-0005.
...
```
frontend/src/
├── auth/            # Cognito SRP auth (cognito.ts), AuthContext, tokenStore (feeds the API client's Authorization header)
├── api/
